...
let STATE = null;
let STATIC = false;
const snapshotNote = () => 'This is the published snapshot. Live actions run in the local app: npm run app';
...
async function load() {
  try {
    ...
    STATIC = true;
    STATE = await (await fetch('static-state.json')).json();
  }
  renderOverview();
  renderHistory();
  renderCompetitors();
...
function renderWorkers() {
  ...
  document.querySelectorAll('[data-worker]').forEach((b) => {
    b.addEventListener('click', async () => {
      if (STATIC) { $('#worker-out').textContent = snapshotNote(); return; }
      b.disabled = true;
...
$('#run-validate').addEventListener('click', async () => {
  if (STATIC) { $('#validate-meta').textContent = snapshotNote(); return; }
...
async function runTo(endpoint, bodyObj) {
  if (STATIC) { out(snapshotNote()); return { }; }
  out('Running…');
